...
class TelnetConnection(telnetlib.Telnet):
    ...
    def write(self, text, loglevel=None):
        ...
        *Note:* This keyword does not return the possible output of the executed
        ...
        # can be used: self._log('123'+newline, loglevel)
        if newline in text:
    ...
    def read_until(self, expected, loglevel=None):
        ...
        # Add comments By Yi Hongyan 
        # Here output the my input CLI command to telnet connection
        self._log(output, loglevel)
        if not success:
    ...
    def read_until_prompt(self, loglevel=None, strip_prompt=False):
        ...
        ''''add by junyan for AP CLI print information 
        '''
        if "--More--" in output:
            flag=True
            outputmsg=""
            while(flag):
                text=" "
                output1=self.write(text, loglevel)
                output2=output1[0:]
                outputmsg=outputmsg+output2
                prompt, regexp = self._prompt
                if prompt in output1:
                    output1=self.write(text, loglevel)
                    output2=output1[36:]
                    outputmsg=outputmsg+output2
                    if prompt in output1:
